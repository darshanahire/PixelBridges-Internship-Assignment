...
function Card1(props) {  
  ...
  return (
    ...
    <div className='card1Parent' style={{background:props.data?.bgColor}} >
        <h1 style={{fontWeight:"400",color:"#d6e1ff"}}>
        {props.data?.headPart1} <span style={{fontWeight:"bold"}}> {props.data?.headPart2} </span>
        </h1>
        <p className='card1Para'>
        {props.data?.para}
        </p>
        <div className='mt-5'>
            <button className='unlockBtn'>{props.data?.btnText}</button>
            <button className='btnWithoutBg'>learn More
